...
# Run the solving process on a separate thread
def solve_sudoku_thread():
    if grid_is_empty():
        messagebox.showinfo("Empty Grid", "Please enter some numbers before solving!")
        return
...
# Wrapper to start the solve process in a new thread
def solve_sudoku():
    solve_thread = threading.Thread(target=solve_sudoku_thread)
    solve_thread.start()
